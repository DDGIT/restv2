...
namespace Com.Aote.ObjectTools
{
    ...
    /// </summary>
    public class PagedObjectList : BasePagedList
    {
        private static Log Log = Log.GetInstance("Com.Aote.ObjectTools.PagedObjectList");
        ...
        public static readonly DependencyProperty MultiPathProperty =
            DependencyProperty.Register("MultiPath", typeof(string), typeof(PagedObjectList),
        ...
        {
            PagedObjectList ol = (PagedObjectList)dp;
        ...
        override public void Load()
        {
            ...
            if (MultiPath != null)
            {
                uri = new Uri(WebClientInfo.BaseAddress + "/" + MultiPath.Replace("%", "%25").Replace("#", "%23").Replace("^", "<").Replace(">", "%3e") + "/" + SumNames + "?uuid=" + uuid);
            }
            else
            {
                uri = new Uri(WebClientInfo.BaseAddress + "/" + Path.Replace("%", "%25").Replace("#", "%23").Replace("^", "<").Replace(">", "%3e") + "/" + SumNames + "?uuid=" + uuid);
            }
        ...
        override public void LoadDetail()
        {
            ...
            string uuid = System.Guid.NewGuid().ToString();
            if (Path == null) return;
            Uri uri = new Uri(WebClientInfo.BaseAddress + "/" + Path.Replace("%", "%25").Replace("#", "%23").Replace("^", "<").Replace(">", "%3e") + "/" + pageIndex + "/" + PageSize + "?uuid=" + uuid);
            WebClient client = new WebClient();
            client.OpenReadCompleted += (o, a) =>
            {
